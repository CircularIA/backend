...
export const getBranch = async (req, res) => {
	try {
		...
		if (findUser.role === "Admin") {
			...
			if (user._id.toString() !== companyUser.ownerUser.toString())
				return res
					.status(400)
					.send({ message: "User is not the owner" });
			//Find the company
			const dataCompany = await Company.findById(
				companyUser._id
			).populate("branches");
			if (!dataCompany)
...
export const registerBranch = async (req, res) => {
	try {
		...
		return res
			.status(200)
			.send({ branch: savedBranch, message: "Branch saved" });
	} catch (error) {
		console.log("error", error);
		if (error.isJoi)
