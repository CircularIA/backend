...
export const createAdminUser = async (req, res) => {
	try {
		...
		if (secretKey != process.env.JWT_KEY)
			return res.status(400).send({ message: "Secret key is not valid" });
		//Create a new user
...
export const createOwnerUser = async (req, res) => {
	try {
		...
		const companyDats = await Company.findById(company).populate(
			"branches"
		);
...
export const createRegularUser = async (req, res) => {
	try {
		...
		const branchesCompany = branches.filter((branch) =>
			companyDats.branches.includes(branch)
		);
		...
		const companyIndicatorsFiltered = companyDats.indicators.filter(
			(indicator) => indicators.includes(indicator._id.toString())
		);

		if (
			companyIndicatorsFiltered.length === 0 ||
			companyDats.indicators.length === 0
		)
