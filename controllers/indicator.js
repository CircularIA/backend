...
const getValue = (name, inputDatsValues) => {
  ...
  if (name === "porcentaje de valorización ciclo biológico") {
    ...
    factorValue =
      (valores["potencialValorizacion"] / valores["entradaResiduosTotales"]) *
      100;
    ...
    factorValue =
      (valores["potencialValorizacion"] * 100) / valores["entradaResiduos"];
...
export const registerIndicator = async (req, res) => {
  ...
  try {
    ...
    if (!result)
      return res.status(400).send({ message: "Failed to register indicator" });
...
export const updateIndicator = async (req, res) => {
  try {
    ...
    if (!result)
      return res.status(400).send({ message: "Failed to update indicator" });
...
export const assignIndicator = async (req, res) => {
  try {
    ...
    const indicatorAssigned = branchExist.indicators.find(
      (item) => item.indicator._id.toString() === indicatorExist._id.toString()
    );
    if (indicatorAssigned)
      return res.status(400).send({ message: "Indicator already assigned" });
